...
__Response__: Thank you for the comment. The purpose of mentioning the other several priors in the previous manuscript is to describe the generalization of the proposed algorithms. In the updated manuscript, we move the subsection describing the other priors to the supporting information so that the readers can focus on understanding the proposed scalable model. We included an additional sentence in the discussion section (see P10 "Moreover, the proposed model can easily generalize to other family of priors or other family of smoothing functions if desired.").
...
__Response__: We thank the review for the comments. For prediction purpose, the proposed model can be flexibly used in the high-dimensional data analysis without any checking or testing. The two-part spike-and-slab LASSO prior automatically detects if the underlying signal is linear or complex. When the signal is linear, the smoothing function of the predictor only includes the linear part; when the signal is complex, cubic spline (implemented in the package) is used to estimate the complex signal. As a supporting evidence, our added simulation demonstrates the proposed model predicts as good as, if not better than, the linear LASSO model, even when signals are linear and sparse (see response to the Comment 1). 
...
In conclusion, the proposed model do not addition detection for prediction purpose, and have different utility for bi-level selection.
