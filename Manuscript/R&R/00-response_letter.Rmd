...
\begin{itemize}
    ...
    \item We streamlined the structure of the manuscript, moving the previous EM-IWLS fitting algorithm subsection to the supporting information for three reasons. First of all, moving can help the audience focus on the model formulation and EM-Coordinate Descent algorithm, avoiding confusion and distraction. Secondly, many useful comments raised by the reviewers related to the EM-IWLS algorithm, e.g. constructing hypothesis testings and confidence band, requires a more dedicated discussion. We are preparing another manuscript discussing those. Lastly, with the added variable selection performance, the manuscript would be length if keeping the EM-IWLS in the main body. The readability of the manuscript would be damaged. Relocating the EM-IWLS section solves the dilemma without hurting the integrity of the current manuscript that emphasize the scalability.
    \item According to Reviewer \#1's and Reviewer \#4's comments, we added linear LASSO model and spikeSlabGAM model among the models to be compared in the simulation study. In addition, we added a new set of simulation scenarios where the functions of predictors are linear.  
