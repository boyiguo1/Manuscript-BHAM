...
idea is to set up a smoothing penalty, depending on the smoothness
penalty matrix via Equation \ref{eq:smoothpen}, in the posterior density
function, or least squared and likelihood function in the penalized
framework. However, direct integration of the smoothness penalty into
the sparsity penalty is not obvious. Given that the smoothness penalty
matrix is most often symmetric and positive semi-definite {[}\^{} we
limit the discusstion to univariate smoothing function. However, the
penalty matrix could possibly contain multiple matrices, e.g.~tensor
product smoother.{]}, the smoothness penalty matrix can be integrated to
the design matrix \(\boldsymbol{X}\) via re-parameterization.
\citet{Marra2011} introduced a re-parameterization of the basis function
matrix \(\boldsymbol{X}\) based on the decomposition of the smoothness
penalty matrix \(\boldsymbol{S}\).

Column-wise centering and re-parameterization introduced in the GAM as
random effect section{[}\^{}5{]} are performed. The re-parameterization
has two purposes. First of all the re-parameterization reshapes the
columns to be linearly independent to each other, and hence, independent
prior can be set to the coefficients. Secondly, the re-parameterization
decomposes the model matrix \(\boldsymbol{X}_j\) for the \(j\)th
covariates to two sub-matrices representing the null space of the spline
and penalized space of the spline, \[
