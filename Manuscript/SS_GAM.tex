...
\begin{itemize}
\item \textcolor{red}{When the linear assumptions are violated, the linear lasso model performs extremely bad. adaptive cosso have improvement upon cosso.}
\item \textcolor{red}{To note, the drastic deteriation in the prediction of mgcv is caused by the model would not converge in those scenarios. A null model were returned as a final model. }
\item \textcolor{red}{Within the simulations where the functional form of the predictors are linear, we see that the proposed model similar to the lasso model regardless of the type outcome, and better moderately better than the model of comparison. (See Table Lnr Gause, and supporting table Lnr-binom.) Particularly, in gaussina simulations, the proposed model shows great advantage compared to SB-GAM (numeralize the advantage), and slightly improvement compare to ssGAM (numeralize the advantage). This shows that the model can be applied without constraining to the assumption where all functional form are non-linear.}
\end{itemize}
