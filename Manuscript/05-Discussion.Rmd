# Discussion
In the paper, we described a novel generalized additive model using Bayesian hierarchical priors, particularly the proposed spike-and-slab spline prior for bi-level functional selection. Meanwhile, we introduced two optimization based algorithms for model fitting. The algorithms can be easily scale up to address high-dimensional data analysis in a computational efficient manner, but also provide naive inferential supports for many commonly used hierarchical priors in the analyses that the sparse assumption is weak or not necessary. Via simulations, we demonstrated that the proposed model provides as good, if not better, prediction performance compared to some state-of-the-art non-linear modeling devices. Compared another newly developed high-dimensional Bayesian GAM that emphasize computational efficiency [@Bai2021], the proposed model provides flexibility to address bi-level functional selection and inferential support while maintaining the same level of prediction accuracy.

For transnational science purpose, the 


Compare to previous Bayesian GAMs, the proposed model greatly increases the scalability to be applied in high-dimensioanl data analyses, while addressing the bi-level selection problem [@Bai2021].
