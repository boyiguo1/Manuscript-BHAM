...
```{r, results = "asis"}
tab_gaus %>% 
  xtable(caption = "The average and standard deviation of the out-of-sample $R^2$ measure for Gaussian outcomes over 50 iterations", label = "tab:gaus") %>% 
  print(comment = FALSE)
```
...
To evaluate the  predictive performance of the models, the statistics caluclated based on the testing dataset, $R^2$ for Gaussian model, area under the curve (AUC) for binomial model,  are calculated <!--and deviance are calculated for Poisson model,--> averaging across 50 simulations. The results are tabulated in Table \ref{tab:gaus}, \ref{tab:bin_auc}.^[is it better to provide a box plot or a table.]
