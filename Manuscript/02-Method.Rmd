...
\begin{equation}
f(\bs y| \bs \beta, \phi) = \prod\limits^n_{i=1}f(y_i|\bs \beta, \phi),\nonumber
\end{equation}
...
E(Y|\bs x) = g^{-1}(a + \sum\limits^p_{j=1} B_j(x_j)) = g^{-1}(a + \sum\limits^p_{j=1} \bs \beta_j^T \bs X_j) = g^{-1}\left[a + \sum\limits^p_{j=1} (\beta_j X^0_j + {\bs \beta_j^\tp}^T \bs X_j^\tp)\right],
\end{equation}
...
\end{equation}
where $S_{j} = (1-\gamma^{0}_{j}) s_0 + \gamma^{0}_{j} s_1$ and $S^\tp_{j} = (1-\gamma^\tp_{j}) s_0 + \gamma^\tp_{j} s_1$. To calculate two unknown quantities $E({S_j}^{-1})$ and $E(S^{-1}_j)$, the posterior probability $p_{j} \equiv \pr(\gamma^{0}_{j}=1|\Theta^{(t-1)})$ and $p_{j}^\tp \equiv \pr(\gamma^\tp_{j}=1|\Theta^{(t-1)})$ are necessary, which can be derived via Bayes' theorem. The calculation of $p_j^\tp$ is slightly different from that of $p_j$, as $p_j^\tp$ depends on the value of the vector $\bs \beta^\tp_{j}$ and $p_j$ only depends on the scalar $\beta_j$. The calculation follows the equations below.
\begin{align*}
