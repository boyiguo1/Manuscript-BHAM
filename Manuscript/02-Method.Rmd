...
A double exponential prior, $\beta|S \sim DE(0, S)$ can be formulated as a hierarchical normal prior with unknown variance $\tau^2 \sim Gamma(1, 1/(2S^2))$ integrated out:
\begin{align*}
  \beta &\sim N(0, \tau^2)\\
  \tau^2 & \sim Gamma(1, 1/(2S^2)), 
\end{align*}
For the mixture double exponential priors, we can define the scale parameter $S = (1-\gamma)s_0 + \gamma s_1$ following Equation (\ref{eq:ssl}). The change in the prior formulation in turns leads to the change in the log posterior density function, as $Q_1$ needs to account for the hyper prior of $\tau^2$: 
...
\end{equation}
Since $\bs \tau^2$ are not of our primary interest, we treat them as the "missing" data in addition to the latent indicators $\bs \gamma$, and hence construct the expectation $E_{\bs \gamma, \bs \tau^2|\Theta^{(t-1)}}(Q_1)$ in the E-step. To note, unlike the same latent indicator $\gamma^\tp_j$ which is shared by the coefficients of the non-linear terms $\beta^\tp_{jk}$ for $k = 1, \dots, K_j$ , $\tau^2_{jk}$ is coefficient specific for $\beta^\tp_{jk}$. $E({S_j^0}^{-1}|\beta_j^0, s_0, s_1), E(S^{-1}_j|\bs \beta_j^\tp, s_0, s_1), E({\tau^0}^2_{j}|S_j^0, \beta^0_j) \text{ and } E({\tau^\tp}^2_{jk}|S_j^\tp, \beta^\tp_{jk})$ needs to be calculated to formulate $E(Q_1)$. As neither $E({S_j^0}^{-1}|\beta_j^0, s_0, s_1)$ nor $E(S^{-1}_j|\bs \beta_j^\tp, s_0, s_1)$ depends on $\tau^2$s, they can be derived using Equation (\ref{eq:exp_scale}) via Bayes' Theorem. On the other hand, $\tau^{2}$ following a gamma distribution is a conjugate prior for the normal variance, and the conditional posterior density of $\tau^{-2}$ is an inverse Gaussian distribution. $E({\tau^0}^{-2}_{j})$ and $E({\tau^\tp}^{-2}_{jk})$ are calculated using the closed form equation
\begin{align*}
 E({\tau^0}^{-2}_{j}|S_j^0, \beta^0_j) ={S_j^0}^{-1}/|\beta^0_j| \qquad E({\tau^\tp}^{-2}_{jk}|S_j^\tp, \beta^\tp_{jk})={S_j^\tp}^{-1}/|\beta^\tp_{jk}|,
\end{align*}
where $S_j^0$ and $S_j^\tp$ are replaced by the expectation and $\beta$s are replaced with $\beta^{(t-1)}$. With simplification (up to constant additive terms), we have 
\begin{equation}\label{eq:EQ1_IWLS}
E(Q_1) = \log f(\textbf{y}|\bs \beta, \phi) + \log f(\phi) - \sum\limits_{j=1}^p\left[ \frac{({\beta_j^0}^2)}{2\phi E{\tau_j^0}^{2}} +\sum\limits_{k=1}^{K_j} \frac{{\beta_{jk}^\tp}^2}{2\phi E({\tau_{jk}^\tp}^{2})}\right].
\end{equation}
$\frac{\beta^2}{2\phi E(\tau^{2})}$ can be seen as the kernel of a normal density with mean 0 and variance $\phi E(\tau^{2})$.
...
$$
  f(\textbf{y}|\bs \beta, \phi) \approx MVN(\textbf{z}|X\beta, \Sigma w_i^{-1}\phi)
$$
...
\end{aligned}
$$
where $L^{'}(y_i|\hat\eta_i, \hat \phi)$ and $L^{''}(y_i|\hat\eta_i, \hat \phi)$ are the first and second derivative of the log density, $\log f(\textbf{y}_i|\bs \beta, \phi)$. With $z\sim N(X\beta, \Sigma\phi)$ and $\beta \sim N(0, \phi\Sigma_{\tau^2})$, we can augment the two normal distributions and derive the estimates for $\bs \beta$ and $\phi$ in each step in closed form equations. To note, an estimate of the variance-covariance matrix can be derived in the EM-IWLS algorithm and in turn can be used for statistical inferences.
