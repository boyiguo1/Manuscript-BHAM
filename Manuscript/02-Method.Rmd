...
To encourage proper smoothing of the functions, we adopt the idea of smoothing penalties from smoothing spline models. The basic idea is to set up a smoothing penalty, described in Equation (\ref{eq:smoothpen}), in the prior density function, or least squared and likelihood function in the penalized framework. However, the direct integration of smoothing penalties into sparsity penalties is not obvious. Marra and  Wood [@Marra2011] proposed a re-parameterization procedure to accommodate the smoothing penalty implicitly. Given the smoothing penalty matrix is most often symmetric and positive semi-definite for univariate smoothing functions, we apply eigendecomposition on the penalty matrix $\bs S = \bs U \bs D \bs U^T$ , where the matrix $\bs D$ is diagonal with the eigenvalues arranged in the ascending order. To note, $\bs D$ can contain $0$ elements on the diagonal, associated with the linear space of the smoothing function. We further write the orthonormal matrix $\bs U \equiv \begin{bmatrix}\bs U^0 : \bs U^{\tp}\end{bmatrix}$ containing the eigenvectors as columns in the corresponding order. That is $\bs U^0$ contains the eigenvectors with zero eigenvalues for the linear space and $\bs U^{\tp}$ contains the eigenvectors (as columns) for the non-zero eigenvalues, i.e. the non-linear space. We multiply the basis function matrix $\bs X$ with the orthonormal matrix $\bs U$ for the new design matrix $\bs X^\ast = \bs X \bs U \equiv \begin{bmatrix} \bs X^0 : \bs X^{\tp} \end{bmatrix}$. An additional scaling step is imposed on $\bs X^{\tp}$ by the non-zero eigenvalues of $\bs D$ such that the new basis function matrix $\bs X^\ast$ can receive uniform penalty on each of its dimensions. With slight abuse of the notation, we denote $\bs X_j \equiv \begin{bmatrix} \bs X_j^0 : \bs X_j^{\tp} \end{bmatrix}$ as the basis function matrix for the $j$th variable after the re-parameterization. A spline function can be expressed in the matrix form
$$
...
\end{equation}
Given that both double exponential distributions have a mean of 0 and the latent indicator $\gamma$ can only take the value of 0 or 1, the mixture double exponential distribution can be formulated as a double exponential density,
...
We introduce a novel prior for GAMs, particularly for high-dimensional nonlinear modeling with bi-level selection. The proposed prior extends from the spike-and-slab lasso prior described in Equation (\ref{eq:ssl}). Given the re-parameterized deign matrix $\bs X_j = \begin{bmatrix} \bs X_j^0 : \bs X_j^\tp \end{bmatrix}$ for the $j$th variable, we impose a two-part SSL prior to the coefficients $\bs \beta_j$. Specifically, we impose independent group priors on the linear space coefficients and on the nonlinear space coefficients respectively, 
...
The same authors [@Rockova2018b; @Rockova2018] further combined the EM algorithm with coordinate descent algorithm for fitting SSL models.
...
* M-step: to maximize the expected likelihood function to calculate the parameter of interest
...
&= \log f(\textbf{y}|\bs \beta, \phi) + \log f(\phi) + \sum\limits_{j=1}^p\left[\log f(\beta^0_j|\gamma^0_j)+\sum\limits_{k=1}^{K_j} \log f(\beta^{\tp}_{jk}|\gamma^{\tp}_{jk})\right]\\
& +\sum\limits_{j=1}^{p} \left[ (\gamma^0_j+\gamma_{j}^{\tp})\log \theta_j + (2-\gamma^0_j-\gamma_{j}^{\tp}) \log (1-\theta_j)\right] +  \sum\limits_{j=1}^{p}\log f(\theta_j)
\end{aligned}
$$  
For simplicity, we consider the linear space to be 1-dimension, which holds true for many smoothing functions.

<!-- , and rewrite $\beta^0_j$ as $\beta_{j0}$, and hence removing the superscript $^{*}$ from the coefficients of the the penalized space. Using the new notation, the part component $\log p(\beta^0_j|\gamma^0_j)+\sum\limits_{k=1}^{K_j} \log p(\beta^{*}_{jk}|\gamma^{*}_{jk})$ can simplifies to $\sum\limits_{k=0}^{K_j} \log p(\beta_{jk}|\gamma_{jk})$. Without loos of generality, the model still work for the spline function whose dimensionality of the null space is more than 1. -->
...
$$
Hereafter, we use the shorthand notation $E(\cdot)\equiv E_{\bs \gamma|\Theta^{(t-1)}}(\cdot)$. In the M-step, we find the $\Theta^{(t)}$ that maximize $E\log f(\Theta, \bs \gamma| \textbf{y}, \textbf{X})$. To note here, the log-posterior density of BHAMs (up to additive constants) can be written as a two-part equation

$$ \log f(\Theta, \bs \gamma| \textbf{y}, \textbf{X}) = Q_1(\bs \beta, \phi) + Q_2 (\bs \gamma,\bs \theta),$$
Where
$$ Q_1(\bs \beta, \phi) = \log f(\textbf{y}|\bs \beta, \phi) + \log f(\phi) + \sum\limits_{j=1}^p\left[\log f(\beta^0_j|\gamma^0_j)+\sum\limits_{k=1}^{K_j} \log f(\beta^{\tp}_{jk}|\gamma^{\tp}_{jk})\right]$$
and $$
Q_2(\bs\gamma,\bs\theta) = \sum\limits_{j=1}^{p} \left[ (\gamma^0_j+\gamma_{j}^{\tp})\log \theta_j + (2-\gamma^0_j-\gamma_{j}^{\tp}) \log (1-\theta_j)\right] +  \sum\limits_{j=1}^{p}\log f(\theta_j) .$$
...
p_{j} &= \frac{\pr(\gamma^{\tp}_{j} = 1|\theta_j)\prod\limits_{k=1}^{K_j}f(\beta_{jk}|\gamma^{\tp}_{j}=1, s_1) }{\pr(\gamma^{\tp}_{j} = 1|\theta_j)\prod\limits_{k=1}^{K_j}f(\beta_{jk}|\gamma^{\tp}_{j}=1, s_1) + \pr(\gamma^{\tp}_{j} = 0|\theta_j)\prod\limits_{k=1}^{K_j}f(\beta_{jk}|\gamma^{\tp}_{j}=0, s_0)}
\end{aligned}
$$
...
&E(\gamma^0_{j})  = p^0_{j} & &E(\gamma^{\tp}_{j}) = p_{j}\nonumber\\
...
E-step: calculate $E(\gamma^0_{j})$, $E(\gamma^{\tp}_{j})$and $E({S^0}^{-1}_{j})$  , $E({S}^{-1}_{j})$with estimates of $\Theta^{(t-1)}$ from previous iteration

M-step:

a) Update $\bs \beta^{(t)}$ and the dispersion parameter $\phi$, if exists, using the coordinate descent algorithm with the penalized likelihood function in Equation (\ref{eq:Q1_CD})
...
Similar to the EM-CD algorithm, the EM-IWLS algorithm is an iterative EM-based algorithm where iterative weighted least square algorithm are used to find the estimate of $\bs \beta, \phi$ that maximizes $E(Q_1)$, instead of the coordinate descent algorithm. It also can fit the models with a wider range of priors, e.g. mixture $t$ prior, including mixture normal prior as an special case, and mixture double exponential prior. The basic idea is to formulate the priors as hierarchical normal distributions and augment with the weighted normal likelihood of the generalized model, such that the model is feasible to fit via least squares algorithms. [@Yi2012] The primary purpose of the EM algorithm here is mitigate the computational burden associate with the hierarchical normal formulation.  

A double exponential prior, $\beta|S \sim DE(0, S)$ can be formulated as a hierarchical normal prior with unknown variances, following gamma distributions, integrated out:
\begin{align*}
  \beta &\sim N(0, \tau^2)\\
  \tau^2 & \sim Gamma(1, 1/(2S^2)), 
\end{align*}
where $S = (1-\gamma)s_0 + \gamma s_1$ for spike-and-slab lasso priors. Now $Q_1$ needs to account for the hyper prior of $\tau^2$ and is expressed as 
\begin{equation}\label{eq:Q1_IWLS}
Q_1(\bs \beta, \phi) = \log f(\textbf{y}|\bs \beta, \phi) + \log f(\phi) + \sum\limits_{j=1}^p\left[\log f(\beta^0_j|{\tau^0}^2_j) + \log f({\tau^0}^2_j| S_j^0)+\sum\limits_{k=1}^{K_j} \{\log f(\beta^{\tp}_{jk}|{\tau^{\tp}}^2_{jk})+\log f({\tau^\tp}^2_{jk}| S^\tp_j)\}\right].
\end{equation}
As $\bs \tau^2$ are not of primary interest, we treat them as the "missing" data besides the latent indicators $\bs \gamma$, and hence need to construct the expectation $E_{\bs \gamma, \bs \tau^2|\Theta^{(t-1)}}(Q_1)$. To note, unlike the coefficients of the non-linear terms $\beta^\tp_{jk}$ which share the same latent indicator $\gamma^\tp_j$, $\tau^2_{jk}$ is coefficient specific for $\beta^\tp_{jk}$. Primarily, four quantities need to be calculate to formulate $E(Q_1), E({S_j^0}^{-1}|\beta_j^0, s_0, s_1), E(S^{-1}_j|\bs \beta_j^\tp, s_0, s_1), E({\tau^0}^2_{j}|S_j^0, \beta^0_j) \text{ and } E({\tau^\tp}^2_{jk}|S_j^\tp, \beta^\tp_{jk})$. As neither $E({S_j^0}^{-1}|\beta_j^0, s_0, s_1)$ nor $E(S^{-1}_j|\bs \beta_j^\tp, s_0, s_1)$ depends on $\tau^2$s, they can be derived via Bayes' theorem as in Equation (\ref{eq:exp_scale}). Shown in Yi and Ma [@Yi2012], the conditional posterior density of $\tau^{-2}$ is an inverse Gaussian distribution, and hence
\begin{align*}
 E({\tau^0}^{-2}_{j}|S_j^0, \beta^0_j) ={S_j^0}^{-1}/|\beta^0_j| \qquad E({\tau^\tp}^{-2}_{jk}|S_j^\tp, \beta^\tp_{jk})={S_j^\tp}^{-1}/|\beta^\tp_{jk}|,
\end{align*}
where $S_j^0$ and $S_j^\tp$ are replaced by the expectation.

To simplify $E(Q_1)$ by removing constant additive terms, we have 
\begin{equation}\label{eq:Q1_IWLS}
E(Q_1(\bs \beta, \phi)) = \log f(\textbf{y}|\bs \beta, \phi) + \log f(\phi) - \sum\limits_{j=1}^p\left[ \frac{({\beta_j^0}^2)E({\tau_j^0}^{-2}}{2\phi} +\sum\limits_{k=1}^{K_j} \frac{({\beta_{jk}^\tp}^2)E({\tau_{jk}^\tp}^{-2}}{2\phi}\right],
\end{equation}
where $\frac{({\beta_{jk}^\tp}^2)E({\tau_{jk}^\tp}^{-2}}{2\phi}$ is the kernel of a normal density with mean 0 and variance $E({\tau_{jk}^\tp}^{-2})\phi$.

Meanwhile, following the classical IWLS, we can approximate the generalized model likelihood at each iteration of the algorithms by the weighted normal likelihood:
$$
  f(\textbf{y}_i|\bs \beta, \phi) \approx Normal(\textbf{z}_i|X_i\beta, w_i^{-1}\phi)
$$
...
\end{aligned}
$$
where $L^{'}(y_i|\hat\eta_i, \hat \phi)$ and $L^{''}(y_i|\hat\eta_i, \hat \phi)$ are the first and second derivative of the log density, $\log f(\textbf{y}_i|\bs \beta, \phi)$. With $z\sim N(X\beta, \Sigma\phi)$ and $\beta \sim N(0, \phi\Sigma_{\tau^2})$, we can augment the two normal distributions and iterative solve for $\bs \beta$ and $\phi$.
