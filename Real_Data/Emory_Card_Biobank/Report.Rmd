...
lasso_mdl_cv <- cv.glmnet(x = train_data %>% select(gam_scn_res %>% pull(var)) %>% data.matrix,
                    y = train_data %>% pull(death3yr), alpha = 1,
                    family = "binomial", type.measure = "auc")

lasso_mdl <- glmNet(x = train_data %>% select(gam_scn_res %>% pull(var)) %>% data.matrix,
                    y = train_data %>% pull(death3yr), alpha = 1,
                    family = "binomial",lambda = lasso_mdl_cv$lambda.min)
...
mdl_defaul <- bmlasso(dsn_mat, train_data$death3yr, family = "binomial", ss  = c(lasso_mdl$prior.scale, 0.5))

mdl_dflt <- bmlasso_spline(dsn_mat, train_data$death3yr, family = "binomial",
                           group = make_group(names(dsn_mat)),
                           ss  = c(lasso_mdl$prior.scale, 0.5))
